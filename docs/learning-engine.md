...
### 3.4 Phase 3: Distillation（N≥K ルール）
...
### 3.5 Phase 4: Application（Silent）
...
### 4.2 Learned Pattern YAML フォーマット
...
11. 注入量の制御（config.yaml の `learning.injection.max_approach` / `max_avoid` / `max_decompose` / `max_tooling` パラメータ）
