# Mutation機構 設計検討 — TANEBI適用分析

> **Status**: Design Document (実装なし)
> **Origin**: claude-crew [mutation_plan.md](https://github.com/skaji18/claude-crew/blob/main/docs/mutation_plan.md) の4層ハイブリッド機構
> **Date**: 2026-02-25
> **Parent**: cmd_085
...
| **L2B** | /mutate Skill (専用ワーカー) | 手動呼び出し | 高 | 85-90% |
...
`config.yaml`の`phase_instructions.execute`に14行のプロンプトを追加。全ワーカー出力に`## Self-Challenge`セクションを義務化。
...
`request.md`内のキーワード（"challenge assumptions", "find flaws", "explore alternatives"等）をDecomposerが検出し、戦略固有の指示をサブタスクに注入。
...
高い親和性。ただし1点注意:
- claude-crewのDecomposerはskillベースで外部設定を読める。TANEBIのDecomposerはテンプレート（プロンプト）のみ
- キーワード→戦略マッピングをテンプレート内にハードコードするか、config.yamlに外出しするか
...
2. **自動キーワード注入**: claude-crewの`/next-round`に相当するもの
...
`templates/worker_reviewer.md`に敵対的レビュープロトコルを追加:
...
最高の親和性。TANEBIのCheckpoint機構はclaude-crewのRed Pen Protocolと同じ目的を持つ。差分はテンプレート内容の強化のみ。
...
### 2.4 Layer 2B: /mutate Skill（専用Mutationワーカー）

#### claude-crewでの実装

専用skill（`.claude/skills/mutate/SKILL.md`）。ユーザーが`/mutate <artifact_path>`で呼び出す。
...
TANEBIにはskillシステムがない。代替案を3つ検討する。
...
- 害: Event Catalogに2イベント追加、flow.pyにハンドラ追加
...
- 害: Checkpointの責務が肥大化、設計の単純さを損なう
...
2. **パス渡し原則との整合**:
   - Mutation WorkerもCorの出力内容を読まない（パスのみ）
...
### 4.1 Skillベース vs フレームワーク

| claude-crew | TANEBI | 差異の影響 |
|-------------|--------|-----------|
| `/mutate` skill（Claude Code内） | APIまたはイベント経由 | L2Bの呼び出し方法が異なる |
| Skillは即座に呼び出し可能 | イベント発行→Executor処理 | TANEBI は非同期（利点でもある） |
| 単一セッション内で完結 | 独立ワーカーとして実行 | TANEBIの方が分離度が高い（sycophancy低減に有利） |

**TANEBIの優位点**: Mutation Workerが完全に独立したプロセスで動くため、原著作ワーカーとの認知的不協和が物理的に排除される。claude-crewでも10%のsycophancyが残ると報告されているが、TANEBIでは別プロセス実行によりさらに低減が期待できる。
...
### 4.3 claude-native vs 外部Listener
...
| **L2B: Mutation Worker** | ⚠️ **条件付き** | コスト高。L0-L2Aで不十分な場合のみ。新イベントタイプ追加が必要。 |

### 5.2 段階的導入計画

claude-crewのProgressive Validation原則をそのまま採用する。
...
2. claude-crewの14行プロンプトをTANEBI向けに調整:
...
claude-crewの4層Mutation機構はTANEBIに**高い親和性**を持つ。特にL0（Self-Challenge）とL2A（Red Pen Protocol）はTANEBIの既存テンプレート・Checkpoint機構と自然に統合できる。
