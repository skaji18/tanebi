...
- **Core 層**: 進化エンジン、Persona Store、フロー決定ロジック。Event Store を介してのみ外部と通信する。Executor の実装を知らない
...
| P2 | Store抽象化 | EventStore / PersonaStore / KnowledgeStore は差し替え可能 |
...
| Evolution Core | エージェント個体の進化 + 共有知識の成長（Section 6 で詳述） | `scripts/evolve.sh` + `scripts/_fitness.py` |
...
| PersonaStore | 人格データの永続化 | KVS（copy / merge / snapshot / list / restore） | ファイルベース (`personas/`) |
| KnowledgeStore | ドキュメント検索 | 検索・取得（今後定義） | ファイルベース (`knowledge/`) |
...
### 3.3 PersonaStore Protocol

人格 YAML の読み書き・バージョン管理。

| メソッド | 説明 |
|---------|------|
| `copy(persona_id, new_id)` | Portable粒度で複製。Performance は白紙スタート |
| `merge(persona_a, persona_b, weights)` | 2体の人格を加重結合し新人格を生成（非破壊） |
| `snapshot(persona_id)` | Full粒度で `personas/history/` に保存 |
| `list()` | アクティブ Persona 一覧 |
| `restore(snapshot_id, persona_id)` | スナップショットから Persona を復元 |
...
### 3.4 KnowledgeStore Protocol

ドキュメント検索操作。**今後定義**。Few-Shot Bank やエピソード記録の検索を担う。
...
| `evolution.started` | Phase 1 対象外。将来の Module 実装時に追加 |
| `evolution.persona_updated` | 同上 |
| `evolution.few_shot_registered` | 同上 |
| `evolution.completed` | 同上 |
...
| Evolution | ui | Persona進化状況・fitness推移の可視化 |
...
進化イベント（`evolution.started`, `evolution.persona_updated`, `evolution.few_shot_registered`, `evolution.completed`）も将来の Module 実装時にイベントカタログに追加する。
