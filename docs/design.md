...
**フロー制御について**: DECOMPOSE→EXECUTE→AGGREGATE→SIGNAL_DETECT→(DISTILL if N≥K) のフロー制御は Core 層の責務（フロー決定ロジック）である。Core が `*.completed` イベントを受けて次の `*.requested` イベントを発行する。
...
events:
  ...
  # --- Checkpoint / Learning Engine イベント ---
  # checkpoint.requested / checkpoint.completed のスキーマは docs/checkpoint-design.md を参照
...
環境別の構成例は `docs/adapter-guide.md` を参照（作成予定）。
...
└── docs/
    ├── design.md                   # 本文書
    ├── adapter-guide.md            # アダプター構成ガイド（環境別config.yaml例）（作成予定）
    └── archive/                    # 旧設計文書アーカイブ
```
