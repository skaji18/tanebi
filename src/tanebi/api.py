...
def submit(request: str, *, work_dir: Path | None = None) -> str:
...
def status(task_id: str, *, work_dir: Path | None = None) -> dict:
...
def result(task_id: str, *, work_dir: Path | None = None) -> str | None:
    """完了していれば report.md の内容を返す。未完了なら None。"""
    if work_dir is None:
        work_dir = Path(WORK_DIR)
    report_path = work_dir / task_id / "report.md"
